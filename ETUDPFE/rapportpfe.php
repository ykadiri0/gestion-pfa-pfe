...
$sql2="SELECT * FROM `affprofetud` WHERE idetud='".$_SESSION['ide']."';";
	    $result2=mysqli_query($data,$sql2);

	    $rows2= mysqli_fetch_assoc($result2);
        $sujet=$rows2['entreprise'];
        ...
        $avatar=$rows3['avatar'];
        $email=$rows3['email'];
        $nom=$rows3['nom']." ".$rows3['prenom'];


if(isset($_POST['update_profile'])){
    mysqli_query($data, "DELETE FROM `rapportpfe` WHERE ide = '".$_SESSION['ide']."'") or die('query failed');

    mysqli_query($data, "INSERT INTO `rapportpfe`(`id`, `ide`, `etudiant`, `entreprise`,description,  `professeur`, `daterapport`) VALUES (null,".$_SESSION['ide'].",'".$_SESSION['nom']." ".$_SESSION['prenom']."','".$_POST['entreprise']."','".$_POST['description']."','$prof','".date("Y/m/d")."')") or die('query failed');
    $update_image = $_FILES['update_image']['name'];
    $update_image_size = $_FILES['update_image']['size'];
     $update_image_tmp_name = $_FILES['update_image']['tmp_name'];
   $update_image_folder = '../storage/rapportpfe/'.$update_image;

   if(!empty($update_image)){
      if($update_image_size > 2000000000000){
         $message[] = 'image is too large';
      }else{
         $image_update_query = mysqli_query($data, "UPDATE `rapportpfe` SET rapport = '$update_image'  WHERE ide = '".$_SESSION['ide']."'") or die('query failed');
        ...
        <div class="col-md-5 border-right">
            <div class="p-3 py-5">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="text-right">Deposer Rapport PFE</h4>
                </div>
                <div class="row mt-2">
                    <div class="col-md-6"><label class="labels">Nom D 'Entreprise </label><input type="text" class="form-control" name="entreprise"  value="<?php echo $sujet?>"></div>
                ...
                <div class="row mt-3">
                <div class="col-md-12"><label class="labels">Description</label><input required name="description" type="text" placeholder="Entrer la description"   class="form-control"  ></div>

                    <div class="col-md-12"><label class="labels">Rapport PFA</label><input  required name="update_image"  type="file" id="actual-btn" class="form-control" >  <!-- our custom upload button -->
